"""Fine-tune a YOLO model (default YOLO11n) as a one-class traffic sign detector.
...
def build_summary(
    *,
    ...
    return {
        "run_name": run_name,
        "model": Path(args.base_model).stem,
...
def print_run_summary(args, best_weights: Path, summary_path: Path, exported_tflite: str | None, baseline_metrics, fine_tuned_metrics, visualizations_dir: Path) -> None:
    print()
    print("=" * 72)
    print(f"{Path(args.base_model).stem} traffic sign detector")
    ...
    print(f"Delta R@50:        {fine_tuned_metrics.recall_at_50 - baseline_metrics.recall_at_50:+.4f}")
    print(f"Baseline F1@50:    {baseline_metrics.f1_at_50:.4f}")
    print(f"Fine-tuned F1@50:  {fine_tuned_metrics.f1_at_50:.4f}")
    print(f"Delta F1@50:       {fine_tuned_metrics.f1_at_50 - baseline_metrics.f1_at_50:+.4f}")
...
def main() -> None:
    ...
    detector_name = Path(args.base_model).stem
    run_name = f"{detector_name}_traffic_sign_{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    DETECTION_MODELS_DIR.mkdir(parents=True, exist_ok=True)

    print(f"Evaluating {detector_name} before fine-tuning...")
    ...
    print(f"Fine-tuning {detector_name}...")
    train_results = train_yolo11n(args, dataset_yaml, run_name)
    # Ultralytics may suffix run_name (e.g. "..._2") if the directory already
    # exists, so derive paths from its actual save_dir rather than run_name.
    ...
    print(f"Evaluating fine-tuned {detector_name}...")
